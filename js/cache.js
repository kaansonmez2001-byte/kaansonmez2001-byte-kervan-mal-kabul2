...
async function reconcile(n,puts=[],deletes=[]){const db=await open();return new Promise((resolve,reject)=>{const tx=db.transaction(n,'readwrite'),store=tx.objectStore(n);for(const value of puts)store.put(value);for(const key of deletes)store.delete(key);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);});}
...
return{open,get,all,put,add,del,clear,indexAll,reconcile,setting,setSetting,log,atomic}})();
